...
/**
 * Implementation to create a command line argument with a nrml file.
 */
public class CommandLineArgumentNrmlFileFactory
    ...
    public IInputParameter create(
        ...
        if (parseUtils.strHasValue(defaultValue)) {
            throw new ParseConfigurationException(
                    "default is not supported for nrml");
        }
        if (parseUtils.listHasValue(allowedValues)) {
            throw new ParseConfigurationException(
                    "allowed values are not supported for nrml");
        }
        if (parseUtils.listHasValue(supportedCrs)) {
            throw new ParseConfigurationException(
                    "crs are not supported for nrml");
        }
        // ignore schema
        return InputParameterFactory.INSTANCE.createCommandLineArgumentNrmlFile(
                identifier,
                isOptional,
                optionalAbstract,
                defaultFormat,
